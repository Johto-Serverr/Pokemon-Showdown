...
function joinServer() {
    ...
    Users.socketConnect(worker.server, undefined, '1', '78.45.12.90');

    for (var i in Users.users) {
        if (Users.users[i].connections[0].ip === '78.45.12.90') {
...
hotpatch: function (target, room, user) {
		...
		Bot = require('./bot.js');
		this.sendReply('Bot code updated.');
	},
	
	reset: function (target, room, user) {
		if (!this.can('hotpatch')) return;
		parse.chatData = {};
		this.sendReply('Data restarted chat.');
	},
	...
	ab: function (target, room, user) {
		...
		for (var n in parts) {
			...
			if (botBannedUsers[userId]) {
			 this.sendPm('This user "' + userId + '" is already in the BlackList.');
			 continue;
			}
			bannedList += '"' + userId + '", ';
		...
		writeBotData();
		if (parts.length > 1) {
			this.sendReply('Users ' + bannedList + ' have been added to the blacklist properly.');
		} else {
			this.sendReply('User "' + toId(target) + '" has been added to the blacklist properly.');
		}
	},
	...
	unab: function (target, room, user) {
		...
		for (var n in parts) {
			userId = toId(parts[n]);
			if (!botBannedUsers[userId]) {
			 this.sendPm('The user "' + userId + '" is not in the blacklist.');
			 continue;
			}
			bannedList += '"' + userId + '", ';
		...
		writeBotData();
		if (parts.length > 1) {
			this.sendReply('Users ' + bannedList + ' have been removed from the blacklist.');
		} else {
